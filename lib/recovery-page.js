...
function pollBanner(){
  api('/api/recovery/status').then(st => {
    ...
    else if (st.pendingRollbacks && st.pendingRollbacks.length) { b.style.display='block'; b.textContent='⚠ 有 '+st.pendingRollbacks.length+' 个恢复回滚未完成（待找回）：已锁定全部业务与恢复点写入，请在下方点击「重试回滚找回数据」成功后再继续。'; }
