...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; }
    ...
    input[type=text],input[type=number],select { border:1px solid var(--line); border-radius:6px; padding:9px; background:#fff; }
    input[type=number]{ width:90px; }
    button { border:0; border-radius:6px; background:var(--accent); color:#fff; padding:9px 14px; font-weight:700; cursor:pointer; }
    ...
    .modal { background:#fff; border-radius:10px; padding:20px; width:min(560px,100%); max-height:88vh; overflow:auto; }
    .modal h2 { margin-top:0; }
    .counts { display:grid; grid-template-columns:repeat(3,1fr); gap:10px; margin:14px 0; }
    .count { border:1px solid var(--line); border-radius:8px; padding:12px; text-align:center; }
    ...
    .changelist { max-height:180px; overflow:auto; border:1px solid var(--line); border-radius:6px; padding:8px 12px; margin:6px 0; font-size:13px; }
    .changelist .empty { color:var(--muted); }
    .warnbox { background:#f6e3dd; border:1px solid #e0b3a6; border-radius:8px; padding:10px 12px; margin:12px 0; color:var(--warn); font-size:13px; }
    ...
    @media (max-width:760px){
      header{padding:16px;} main{padding:14px;}
...
function render(){
  ...
  rowsEl.innerHTML = pts.slice().reverse().map(p => {
    ...
    const actions = '<div class="row-actions">'
      ...
      + '</div>';
    return '<tr>'
      + '<td><div>'+fmtTime(p.createdAt)+'</div><div class="meta">'+esc(p.id)+(p.note?' · '+esc(p.note):'')+'</div></td>'
      + '<td>'+kindPill(p)+(p.pinned?' <span class="tag-pin">📌</span>':'')+'</td>'
      + '<td class="meta">'+chain+'</td>'
      + '<td>'+status+'</td>'
      + '<td class="meta">'+content+'</td>'
      + '<td>'+actions+'</td>'
      + '</tr>';
