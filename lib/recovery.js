...
export class RecoveryEngine {
  ...
  async restore({ restoreId, pointId, backupContent, writeDb, inject }) {
    return this.mutex.run(async () => {
      ...
      if (existing) {
        ...
        if (existing.status === "rollback_failed")
          throw new RecoveryError(
            409,
            "rollback_incomplete",
            "此前恢复失败且自动回滚也未成功，恢复前数据仍在备份中，请先重试回滚找回数据",
            { journal: existing }
          );
        throw new RecoveryError(
          409,
      ...
      const rollback = async (error, crashed = false) => {
        ...
        if (rollbackError) {
          // 回滚本身没写成功：绝不能宣告已回滚，更不能删备份——保留恢复前数据以便重试找回
          const failedJournal = {
            ...journal,
            finishedAt: this.now(),
          ...
          await atomicWrite(this.journalFile(restoreId), JSON.stringify(failedJournal, null, 2));
      ...
      try {
        ...
        if (finalJournal.status === "rollback_failed")
          throw new RecoveryError(
            500,
            "rollback_incomplete",
            "恢复失败且自动回滚未成功，恢复前数据已保留在备份中，可重试回滚找回",
   ...
   *   若回滚写入也失败，置 rollback_failed 并保留备份，等待磁盘恢复后重试找回；
   * - rollback_failed：再次尝试用备份回滚；
  ...
  async replayJournals({ writeDb }) {
    return this.mutex.run(async () => {
      ...
      for (const j of journals) {
        if (j.status !== "in_progress" && j.status !== "rollback_failed") continue;
        const backupPath = this.backupFile(j.restoreId);
        if (!existsSync(backupPath)) {
          // 备份还没写就中断：数据未动，仅终结日志
          const done = { ...j, finishedAt: this.now(), status: "rolled_back", crashed: true, error: j.error || "interrupted_before_swap", rollbackError: null };
        ...
        if (backup) {
          ...
          writeError = "backup_unreadable";
        }
        if (writeError) {
          // 回滚仍失败：保持/置为 rollback_failed 且不删备份，磁盘恢复后可再次重试找回恢复前数据
          const failed = {
            ...j,
            finishedAt: this.now(),
            status: "rollback_failed",
            crashed: true,
            error: j.error || "interrupted_during_restore",
            rollbackError: writeError,
          };
          await atomicWrite(this.journalFile(j.restoreId), JSON.stringify(failed, null, 2));
          recovered.push({
        ...
        recovered.push({ restoreId: j.restoreId, pointId: j.pointId, action: "rolled_back" });
      }
      return { recovered };
    });
  }

  /** 需要重试回滚的中断恢复（rollback_failed）列表 */
  async listPendingRollbacks() {
    const journals = await this.listJournals();
    return journals.filter((j) => j.status === "rollback_failed");
  }

  /**
   * 磁盘恢复后手动重试回滚：把备份（恢复前数据）重新写回。
   * 成功才删除备份并终结日志；失败继续保留备份，可再次重试。
   */
  async recoverRollback(restoreId, { writeDb }) {
    return this.mutex.run(async () => {
      this.validateRestoreId(restoreId);
      const journal = await this.getJournal(restoreId);
      if (!journal) throw new RecoveryError(404, "journal_not_found", "恢复日志不存在");
      if (journal.status !== "rollback_failed")
        throw new RecoveryError(409, "rollback_not_pending", "该恢复没有待重试的失败回滚", { status: journal.status });
      const backupPath = this.backupFile(restoreId);
      if (!existsSync(backupPath))
        throw new RecoveryError(409, "backup_missing", "备份文件已不存在，无法找回恢复前数据");
      const backup = await readJson(backupPath, null);
      if (!backup) throw new RecoveryError(500, "backup_unreadable", "备份文件无法读取");
      await writeDb(backup);
      const done = {
        ...journal,
        finishedAt: this.now(),
